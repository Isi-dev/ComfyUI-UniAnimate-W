...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 ...
                 use_checkpoint=True, pose_attention = False,para = False, our_weight = 1):
        super().__init__()
        ...
        if pose_attention:
            self.pose_attention = SpatialTransformer_pose(
                                    in_channels, n_heads, d_head, depth = depth, context_dim= context_dim, disable_self_attn = disable_self_attn, use_linear = use_linear, para =para,our_weight = self.our_weight
                                )


    def forward(self, x, context=None, pose_embedding = None):
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            # if context[i]!=None:
            #     print("no _p elf.transformer_blocks_ context[i].shape===>>>", x.shape, context[i].shape) # torch.Size([33, 81, 1024])
            # else:
            #     print("no  _p  elf.transformer_blocks_ context[i].shape===>>> is none")
        ...
        if not self.use_linear:
            x = self.proj_out(x)
        x = x + x_in
        if self.pose_attention == False:
            return x
        elif self.para == False:
            x = self.pose_attention(x, pose_embedding )
            # print('here')
            return x
        else:

            x = x + self.pose_attention(x_in, pose_embedding )
            # print('here')
            return x
class SpatialTransformer_pose(nn.Module):
    """
    Transformer block for image-like data.
    First, project the input (aka embedding)
    and reshape to b, t, d.
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0., context_dim=None,
                 disable_self_attn=False, use_linear=False,
                 use_checkpoint=True, para = False, our_weight = 1):
        super().__init__()
        if exists(context_dim) and not isinstance(context_dim, list):
            context_dim = [context_dim]
        self.in_channels = in_channels
        inner_dim = n_heads * d_head
        self.our_weight = our_weight
        self.norm_p = torch.nn.GroupNorm(num_groups=32, num_channels=in_channels, eps=1e-6, affine=True)
        if not use_linear:
            self.proj_in_p = nn.Conv2d(in_channels,
                                     inner_dim,
                                     kernel_size=1,
                                     stride=1,
                                     padding=0)
        else:
            self.proj_in_p = nn.Linear(in_channels, inner_dim)

        self.transformer_blocks_p = nn.ModuleList(
            [BasicTransformerBlock(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim[d],
                                   disable_self_attn=disable_self_attn, checkpoint=use_checkpoint)
                for d in range(depth)]
        )
        if not use_linear:
            self.proj_out_p = zero_module(nn.Conv2d(inner_dim,
                                                  in_channels,
                                                  kernel_size=1,
                                                  stride=1,
                                                  padding=0))
        else:
            self.proj_out_p = zero_module(nn.Linear(in_channels, inner_dim))
        self.use_linear = use_linear
        self.para = para
    ...
    def forward(self, x, context=None):
        ...
        x = rearrange(x, 'b c h w -> b (h w) c').contiguous()
        if self.use_linear:
            # print("before  = self.proj_in_ x.shape===>>>", x.shape)
            x = self.proj_in_p(x)
            # print("after  = self.proj_in_ x.shape===>>>", x.shape)
        for i, block in enumerate(self.transformer_blocks_p):
            # if context[i]!=None:
            #     print("_p elf.transformer_blocks_ context[i].shape===>>>", context[i].shape)
            # else:
            #     print("_p  elf.transformer_blocks_ context[i].shape===>>> is none")
            # print("before  = block(x, c x.shape===>>>", x.shape)
            
            x = block(x, context=context[i])
            # print("after  = block(x, c x.shape===>>>", x.shape)
        if self.use_linear:
            # print("before  =x = self.proj_out x.shape===>>>", x.shape)
            x = self.proj_out_p(x)
            # print("after  =x = self.proj_out x.shape===>>>", x.shape)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        if not self.use_linear:
            x = self.proj_out_p(x)
        if self.para:
            ...
            # print('---------x*self.self.our_weight + x_in: ---------', self.our_weight)

            return x*self.our_weight + x_in
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        # if context==None:
        #     print("context==None: x.shape: ", x.shape)
        # else:
        #     print("context!=None: x.shape: ", x.shape, "context.shape:", context.shape) # context!=None: x.shape:  torch.Size([33, 6144, 320]) context.shape: torch.Size([33, 81, 1024])
        # x = self.attn2(self.norm2(x), context=context) + x
        x_tmp = self.norm2(x)
        # print(self.norm2)
        # if context == None:
        ...
        # else:
        #     print("x_tmp.shape, context is not none", x_tmp.shape) 
        # print(self.attn2) # 目前的：torch.Size([33, 6144, 320]) ； torch.Size([1536, 33, 640])  torch.Size([6144, 33, 512])  orch.Size([6144, 33, 320]) 
        x = self.attn2(x_tmp, context=context) + x # _tmp.shape, context is none torch.Size([33, 6144, 320])
